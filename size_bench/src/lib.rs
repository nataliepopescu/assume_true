...
#[no_mangle]
pub fn c_style_input_u64_fixed(input: &[u64; 40000], output: &mut [u64]) {
    for i in 0..input.len() {
        output[i] = input[i];
    }
}
...
#[no_mangle]
pub fn c_style_input_u32_fixed(input: &[u32; 80000], output: &mut [u32]) {
    for i in 0..input.len() {
        output[i] = input[i];
    }
}
...
#[no_mangle]
pub fn c_style_input_u16_fixed(input: &[u16; 160000], output: &mut [u16]) {
    for i in 0..input.len() {
        output[i] = input[i];
    }
}
