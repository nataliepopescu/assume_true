...
*/
            #[inline(never)]
            pub fn optimal_unsafe_impl(output: &mut [u8; 640000], input: &[u16; 320000]) {
...
*/
            #[inline(never)]
            pub fn c_style_fixed_size_impl(output: &mut [u8; 640000], input: &[u16; 320000]) {
                for i in 0..input.len() {
                    let b = input[i];
                    output[2 * i] = (b & 0xff) as u8;
                    output[2 * i + 1] = ((b >> 8) & 0xff) as u8;
...
*/
            ...
            //#[assume_true(output.len() == 2 * input.len())]
            //#[assume_true(output.len() == 640000)]
            pub fn c_style_input_size_fixed_impl(output: &mut [u8], input: &[u16; 320000]) {
                for i in 0..input.len() {
                    let b = input[i];
                    output[2 * i] = (b & 0xff) as u8;
                    output[2 * i + 1] = ((b >> 8) & 0xff) as u8;
...
*/
            ...
            //  OPT:
            //#[assume_true(input.len() == 320000)]
            pub fn c_style_output_size_fixed_impl(output: &mut [u8; 640000], input: &[u16]) {
                for i in 0..input.len() {
                    let b = input[i];
                    output[2 * i] = (b & 0xff) as u8;
                    output[2 * i + 1] = ((b >> 8) & 0xff) as u8;
...
*/
            ...
            pub fn c_style_unknown_size_impl(output: &mut [u8], input: &[u16]) {
                for i in 0..input.len() {
                    let b = input[i];
                    output[2 * i] = (b & 0xff) as u8;
                    output[2 * i + 1] = ((b >> 8) & 0xff) as u8;
...
*/
            #[inline(never)]
            //#[assume_true(input.len() == 320000 && output.len() == 640000)]
            pub fn c_style_unknown_size_limit_impl(output: &mut [u8], input: &[u16]) {
                use std::cmp::max;
                for i in 0..max(320000, input.len()) {
                    let b = input[i];
                    output[2 * i] = (b & 0xff) as u8;
...
*/
            #[inline(never)]
            pub fn zip_chunks_fixed_size_impl(output: &mut [u8; 640000], input: &[u16; 320000]) {
                for (&b, ac) in input.iter().zip(output.chunks_mut(2)) {
                    // the type of `ac` is `[u8]`, but the optimizer should be able to figure out that it's
                    // always size 2, and thus should elide the bounds checks in the inner loop.
                    ac[0] = (b & 0xff) as u8;
...
/*
        pub mod zip_chunks_fixed_size_take {
            ...
            #[inline(never)]
            pub fn zip_chunks_fixed_size_take_impl(output: &mut [u8; 640000], input: &[u16; 320000]) {
                for (&b, ac) in input.iter().zip(output.chunks_mut(2)).take(320000) {
                    // the type of `ac` is `[u8]`, but the optimizer should be able to figure out that it's
                    // always size 2, and thus should elide the bounds checks in the inner loop.
                    ac[0] = (b & 0xff) as u8;
...
*/
            ...
            pub fn zip_chunks_fixed_size_take_iter_impl(output: &mut [u8; 640000], input: &[u16; 320000]) {
                for (&b, ac) in input.iter().zip(output.chunks_mut(2)).take(320000) {
                    let mut val = b;
...
*/
            #[inline(never)]
            //#[assume_true(input.len() == 320000)]
            pub fn zip_chunks_output_size_fixed_impl(output: &mut [u8; 640000], input: &[u16]) {
                for (&b, ac) in input.iter().zip(output.chunks_mut(2)) {
                    // the type of `ac` is `[u8]`, but the optimizer should be able to figure out that it's
                    // always size 2, and thus should elide the bounds checks in the inner loop.
                    ac[0] = (b & 0xff) as u8;
...
*/
            #[inline(never)]
            //#[assume_true(output.len() == 640000)]
            pub fn zip_chunks_input_size_fixed_impl(output: &mut [u8], input: &[u16; 320000]) {
                for (&b, ac) in input.iter().zip(output.chunks_mut(2)) {
                    // the type of `ac` is `[u8]`, but the optimizer should be able to figure out that it's
                    // always size 2, and thus should elide the bounds checks in the inner loop.
                    ac[0] = (b & 0xff) as u8;
...
*/
            #[inline(never)]
            //#[assume_true(output.len() == 640000 && input.len() == 320000)]
            pub fn zip_chunks_unknown_size_impl(output: &mut [u8], input: &[u16]) {
                for (b, ac) in input.iter().zip(output.chunks_mut(2)) {
                    ac[0] = (*b & 0xff) as u8;
                    ac[1] = ((*b >> 8) & 0xff) as u8;
                }
...
*/
            #[inline(never)]
            //#[assume_true(output.len() == 640000 && input.len() == 320000)]
            pub fn zip_chunks_unknown_size_take_impl(output: &mut [u8], input: &[u16]) {
                for (b, ac) in input.iter().zip(output.chunks_mut(2)).take(320000) {
                    ac[0] = (*b & 0xff) as u8;
                    ac[1] = ((*b >> 8) & 0xff) as u8;
                }
...
*/
            #[inline(never)]
            //#[assume_true(output.len() == 640000 && input.len() == 320000)]
            pub fn zip_chunks_unknown_size_take_iter_impl(output: &mut [u8], input: &[u16]) {
                for (&b, ac) in input.iter().zip(output.chunks_mut(2)).take(320000) {
                    let mut val = b;
                    for byte in ac.iter_mut() {
                        *byte = (val & 0xFF) as u8;
...
*/
            #[inline(never)]
            //#[assume_true(output.len() == 640000 && input.len() == 320000)]
            pub fn zip_chunks_exact_unknown_size_impl(output: &mut [u8], input: &[u16]) {
...
*/
            #[inline(never)]
            //#[assume_true(output.len() == 640000 && input.len() == 320000)]
            pub fn zip_chunks_exact_unknown_size_slice_impl(output: &mut [u8], input: &[u16]) {
                use std::cmp::min;
                for (b, ac) in input[..(min(320000, input.len()))]
                    .iter()
                    .zip(output.chunks_exact_mut(2)) {
...
*/
            #[inline(never)]
            //#[assume_true(output.len() == 640000 && input.len() == 320000)]
            pub fn zip_chunks_exact_unknown_size_take_impl(output: &mut [u8], input: &[u16]) {
...
*/
            #[inline(never)]
            //#[assume_true(output.len() == 640000 && input.len() == 320000)]
            pub fn zip_chunks_exact_unknown_size_take_iter_impl(output: &mut [u8], input: &[u16]) {
                for (&b, ac) in input.iter().zip(output.chunks_exact_mut(2)).take(320000) {
                    let mut val = b;
                    for byte in ac.iter_mut() {
                        *byte = (val & 0xFF) as u8;
